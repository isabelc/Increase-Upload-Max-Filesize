...
class Increase_Upload_Max_Filesize {
	...
	*/
	...
	public function create_admin_page(){ ?>
		...
		if ( $d < 5 ) {
			...
			$status .= sprintf(__( 'Please allow some time for your server to recognize any changes. You can refresh this page in a few minutes to see if any changes have been made.%1$s You may deactivate the plugin once your %2$supload_max_filesize%3$s is at least 32M.%1$s', 'inc-upload-max-filesize' ), '<br /><br />', '<code>', '</code>' );
		...
		<form method="post" action="options.php">
	        <?php settings_fields('inc_upload_max_filesize_options');	// @param 1 must be same as register settings'
				...
				submit_button('Run The Plugin Once'); ?>
		</form>
		</div>
		<?php
    }
	...
	*/
	public function page_init(){	
		...
		add_settings_section(
			...
			__('RUN THE PLUGIN USING php.ini INSTEAD OF php5.ini', 'inc-upload-max-filesize' ),
	...
	* @since 1.0.1
	*/
    public function run_on_save(){
		$options = get_option( 'inc_upload_max_filesize_options' );
		$ischecked = $options['checkbox_one'];
		if( 'on' == $ischecked ) {
			$this->ini_rules();
		}
	}
	...
	*/
	public function ini_rules() {
		...
		if ( $f < 32 ) {
			...
			$msg .= sprintf( __( 'Your upload_max_filesize was already %s so it will not be changed.%s', 'inc-upload-max-filesize' ), $d, '<br />');
		}
		...
		if ( $c < 64 ) {
			...
			$msg .= sprintf( __( 'Your memory_limit was %s.%s', 'inc-upload-max-filesize' ), $a, '<br />');
		} else {
			$msg .= sprintf( __( 'Your memory_limit was already %s so it will not be changed.%s', 'inc-upload-max-filesize' ), $a, '<br />');
		}
		...
		if ( $z < 33 ) {
			...
			$msg .= sprintf( __( 'Your post_max_size was already %s so it will not be changed.%s', 'inc-upload-max-filesize' ), $x, '<br />');
		}
